...
<link href="${pageContext.request.contextPath}/css/dash.css" rel="stylesheet" />
</head>
<body>
                        ...
                        <div class="d-flex justify-content-around fs-5">
                            <div>
                                ...
                                <div class="fw-bold text-dark">
                                    ¥<fmt:formatNumber value="${yearlyTotal}" type="number" groupingUsed="true" />
                                </div>
                            </div>
                            <div>
                                <div class="text-muted">月間</div>
                                <div class="fw-bold text-primary">
                                    ¥<fmt:formatNumber value="${monthlyTotal}" type="number" groupingUsed="true" />
                                </div>
                            </div>
                        </div>
                    </div>
                        ...
                        <div class="d-flex justify-content-around fs-5">
                            <div>
                                ...
                                <div class="fw-bold text-success">
                                    ¥<fmt:formatNumber value="${yearlyUserTotal}" type="number" groupingUsed="true" />
                                </div>
                            </div>
                            <div>
                                <div class="text-muted">月間</div>
                                <div class="fw-bold text-success">
                                    ¥<fmt:formatNumber value="${monthlyUserTotal}" type="number" groupingUsed="true" />
                                </div>
                            </div>
                        </div>
                    </div>
        ...
        <script>
            const monthlyCategoryData = {
                <c:forEach var="entry" items="${monthlyCategoryData}" varStatus="loop">
                    "${entry.key}": [
                        <c:forEach var="val" items="${entry.value}" varStatus="status">
